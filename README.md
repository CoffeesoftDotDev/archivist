...
Run Archivist with `uvx`, without installing it. This first run only shows what would change:

```bash
uvx --from git+https://github.com/CoffeesoftDotDev/archivist archivist --parent-folder "C:\photos"
```

To get a permanent `archivist` command, install it as a uv tool, then apply the changes once the preview looks right:
...
| `--apply`                              | off                          | Make the changes. Without it, Archivist only shows what it would do. See [Dry run](#dry-run) |
...
* Without `--apply`, `--send-to-bin` only changes how removals are described.

> [!NOTE]
> In v0.1, runs changed files by default and `--dry-run` turned that off. `--dry-run` still works, and still wins over `ARCHIVIST_APPLY=true`, but it's no longer needed. `ARCHIVIST_DRY_RUN` is ignored, with a warning in the report.

## Examples

```bash
# Preview what would happen (the default)
...
archivist --parent-folder "C:\photos" --apply --leave-eadir
...
# Preview with the defaults
docker run --rm -v "C:\photos:/data" archivist

# Apply the changes, configured with environment variables
docker run --rm -v "C:\photos:/data" -e ARCHIVIST_APPLY=true -e ARCHIVIST_LEAVE_ZIP=true -e ARCHIVIST_SEND_TO_BIN=true archivist

# Append options, which override the environment variables
docker run --rm -v "C:\photos:/data" archivist --apply --max-size 4096
```

With Docker Compose, copy [.env.example](.env.example) to `.env`, set `HOST_PARENT_FOLDER` and any `ARCHIVIST_*` values (`ARCHIVIST_APPLY=true` to make changes), then run:
